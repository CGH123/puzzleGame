...
public class Mountain implements Draw {
    //单位长度
    ...
    //顶点数量
    int vCount = 0;
    int texId, rock_textId;
    int rows, cols;

    public float[][] yArray;
    public final float LAND_HIGH_ADJUST = 2f;//陆地的高度调整值
    public final float LAND_HIGHEST = 60f;//陆地最大高差

    public Mountain(Bitmap bt, int texId, int rock_textId) {
        yArray = loadLandforms(bt);
        rows = yArray.length - 1;
        cols = yArray[0].length - 1;
        this.texId = texId;
        this.rock_textId = rock_textId;
    ...
    //从灰度图片中加载陆地上每个顶点的高度
    public float[][] loadLandforms(Bitmap bt) {
